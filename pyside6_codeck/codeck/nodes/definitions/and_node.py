...
def output_code(node, build_pin_var_name, get_connection_input, get_connection_exec_output):
    ...
    if input1 is None:
        input1 = str(node.data.get('input1', False)).lower()
    
    input2 = get_connection_input('input2')
    if input2 is None:
        input2 = str(node.data.get('input2', False)).lower()
    
    return f'({input1} && {input2})'


AndNodeDefinition = CodeckNodeDefinition(
    name='and',
    label='And (&&)',
    node_type='logic',
    width=width,
    ...
    inputs=[
        port_pin_input('input1', width, 1, input_type='boolean'),
        port_pin_input('input2', width, 2, input_type='boolean')
    ],
    outputs=[
